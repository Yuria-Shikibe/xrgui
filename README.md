...
XRGUI(mo_yanXi's Retaine mode GUI)是一个近似**每帧重绘和局部更新**的带有**自动**布局的支持多个绘制后端（目前只实现了Vulkan)、*（截至目前）不适合用于生产*、特性使用激进（主要为语言特性）、力求轻量化的跨平台和支持所有主流编译器的纯粹面向命令式代码编写的**保留式GUI**
...
6. 不提供声明式语法和反序列化式的UI构建，如有需要自行编写。待C++26反射可用后
