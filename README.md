...
### 设计哲学：
*（截至目前）不适合用于生产、移动端（至少对于默认后端来说如此）*、特性使用激进（主要为语言特性，如C++23模块，Vulkan其实用的扩展也不少）
...
* ***你永远不应该动态链接本库***(由于某些类型擦除实现的缘故)，更激进地说，你应该永远从源码构建此库。
...
## 布局说明
[布局使用文档](properties/showcase/layout_doc.md)

[布局实现文档](properties/showcase/layout_spec.md)
...
目前库还处于实验阶段，暂时不提供通过xrepo的直接包安装，建议直接拉成submodule或者clone下来

* WIN下不要把项目放在很深的文件夹嵌套层级中
* 需要最新版的MSVC(截至文档更新需要14.52)
* 需要你已安装最新版的Vulkan SDK
* 注意：`gen_slang`和`gen_icon`都需要python环境，可能还需要额外安装包
* 注意：`gen_slang`需要你的PATH内有可用的*slangc*，或者你可以自行添加task的参数指定*slangc*路径
* 注意：`gen_icon`需要*Node.js*，尽管我默认多数开发者的电脑上会有Node.js，但如果你有更好的能减少相关依赖的svg工具请留言issue，感激不尽。内建的icon是通过代码生成头文件直接硬编码进样例代码的，所以不可跳过这个步骤。
