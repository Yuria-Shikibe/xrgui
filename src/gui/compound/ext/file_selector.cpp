...
void set_elem_style_(elem& e, std::string_view name){
	action::push_runnable_action(e, [name](elem& el){
...
bool file_selector::cared_file(const path& p) const noexcept{
	...
	return (txt.empty() || p.filename().u32string().contains(txt)) &&
		(std::filesystem::is_directory(p) || is_suffix_met(p));
}
...
void file_selector::build_trace_() noexcept{
	...
	try{
		...
		if(last){
			last->_debug_identity = 12;
		}

		if(last && std::ranges::none_of(
			std::filesystem::directory_iterator{root},
			static_cast<bool(std::filesystem::directory_entry::*)() const>(&
