...
namespace style{
struct scroll_pane_bar_drawer : style_drawer<scroll_pane>{
...
protected:
	math::raw_frect get_scroll_region(const scroll_pane& element, float isHori){
	...
	template <std::invocable<math::raw_frect, bool> BarConsumer, std::invocable<math::raw_frect, bool> LockBarConsumer>
	void each_scroll_rect(const scroll_pane& element, math::frect region, BarConsumer barConsumer,
		...
		if(logicHori || logicVert){
			...
			auto draw_rect = [&](bool isHori, bool active){
				...
				if(active){
					math::raw_frect bar_rect = isHori ? element.get_hori_bar_rect() : element.get_vert_bar_rect();
	...
	void each_scroll_rect(const scroll_pane& element, math::frect region, BarConsumer barConsumer) const{
		this->each_scroll_rect(element, region, barConsumer, barConsumer);
	}


	void draw_layer_impl(const scroll_pane& element, math::frect region, float opacityScl,
