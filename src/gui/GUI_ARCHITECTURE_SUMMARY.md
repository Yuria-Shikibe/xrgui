...
### 2.2 布局协商过程
1. **获取期望尺寸 (`pre_acquire_size`)**：子元素首先被询问其所需要的空间大小。这通常在父元素无法直接确定子元素大小（比如自动换行、网格自适应）时使用。
2. **分配实际尺寸 (`resize`)**：父元素在确定所有子元素的占位后，通过调用子元素的 `resize(vec2)` 或利用 `basic_cell` 应用分配的区域。如果元素实际大小改变，将会触发自身以及可能的子/父级重排标记。
3. **定位与变换**：
...
* **`scene` (场景管理器)**：
  ...
  * 负责分发最顶级的输入事件、运行全局的更新循环（`update` 和 `draw`）、管理全局焦点（聚焦的键盘、鼠标元素）以及弹出层（Overlay/Tooltip）。
* **`elem` (UI 元素基类)**：
  * 所有可交互和可绘制对象的基类。
