...
static_assert(sizeof(guard_unit_head) == 16, "guard_unit_head must be exactly 16 bytes");
...
public:
    ...
    void each(const stack_argument_t& initial_param){
        ...
        *param_stack_pointer = initial_param;
        ...
        try {
            ...
            // [异常恢复] 尽力消耗剩余所有的 guards
            while (current_guard_head_pos != invalid_guard_pos) {
                try {
                    guard_on_exit(invalid_guard_pos);
                } catch (...) {
                    // 如果 guard 内部引发异常，吞下此异常，确保持续清理栈
                }
            }
	...
	template <bool AssumeRelocatableThoughUB = false, std::invocable<> Fn>
	void push_guard(Fn&& fn){
	    using FnTy = std::decay_t<Fn>;
        static_assert(AssumeRelocatableThoughUB || (std::is_trivially_copyable_v<FnTy> && std::is_trivially_destructible_v<FnTy>), "guard function must be trivially copiable, or **trivially relocatable**");
...
private:
    void guard_on_exit(std::uint32_t target_pos){
        ...
        while (current_guard_head_pos != target_pos && current_guard_head_pos != invalid_guard_pos) {
            ...
            // [核心修复] 在可能引发异常的操作前更新游标，确保抛出异常时状态推进不会卡死
