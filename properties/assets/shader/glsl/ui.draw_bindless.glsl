...
dispatch_config_0 dbh_input_get_dispatch_config_0(dbh_input_0 this_0)
{
    ...
    return _slang_resource_heap_11[uvec2(this_0.dispatch_buffer_start_index_0 + 5U, 0U).x]._data[0];
}
...
void main()
{
    if (gl_LocalInvocationIndex == 0) {
        ...
        for (uint i = 0; i < active_primitives; ++i) {
            ...
            // 写入顶点位置
            gl_MeshVerticesEXT[v0].gl_Position = vec4(offset_x, scale, 0.0, 1.0);
            gl_MeshVerticesEXT[v1].gl_Position = vec4(offset_x - scale, -scale, 0.0, 1.0);
            gl_MeshVerticesEXT[v2].gl_Position = vec4(offset_x + scale, -scale, 0.0, 1.0);
...
//
//        // ==========================================
//        // 1. 写入顶点数据 (Vertex Data)
//        // ==========================================
//
//        // 顶点 0: 顶部 (红色)
//        gl_MeshVerticesEXT[0].gl_Position = vec4(0.0, 0.5, 0.0, 1.0);
//        verts_vtx_color_0[0] = color;
//        verts_vtx_uv_0[0] = vec2(0.5, 1.0);
//
//        // 顶点 1: 左下角 (绿色)
//        gl_MeshVerticesEXT[1].gl_Position = vec4(-0.5, -0.5, 0.0, 1.0);
//        verts_vtx_color_0[1] = color;
//        verts_vtx_uv_0[1] = vec2(0.0, 0.0);
//
//        // 顶点 2: 右下角 (蓝色)
//        gl_MeshVerticesEXT[2].gl_Position = vec4(0.5, -0.5, 0.0, 1.0);
//        verts_vtx_color_0[2] = color;
//        verts_vtx_uv_0[2] = vec2(1.0, 0.0);
//
//        // ==========================================
//        // 2. 写入图元数据 (Primitive Data)
//        // ==========================================
//
//        // 组装三角形：连接顶点 0, 1, 2 (逆时针顺序)
//        gl_PrimitiveTriangleIndicesEXT[0] = uvec3(0, 1, 2);
//
//        // 设置每图元 (per-primitive) 自定义属性
//        prims_texture_index_0[0] = 0U; // 硬编码纹理索引为 0
//
//    }
